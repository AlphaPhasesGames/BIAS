...
public class WoodWindowSave : MonoBehaviour
{
    [Header("DoorDetails")] // header for the  Rotated bricks details
    public string windowName; // string to store the Rotated brick name, players can edit
    public Rigidbody rB;
    private void Awake() // on awake
    {
        // SaveSystem.cementBrick.Add(this); // add this otated brick to the list of saved objects
        SaveSystem.woodWindow.Add(this); // add this otated brick to the list of saved objects
    }
    ...
    private void OnMouseDown() // if mouse is over this object and clicked
    {
        ...
        {
            SaveSystem.woodWindow.Remove(this); // remove it from the list of rotated bricks, so it doesnt load back in next time we start the game.
