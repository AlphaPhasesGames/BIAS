...
public class PlayerController : MonoBehaviour
{
    ...
    public GameObject woodenWindowToBuild;
    public GameObject woodenWindowToBuildLayer2;
    ...
    public GameObject transWoodWindow;
    ...
    private void Awake() // on awake of script
    {
        ...
        transWoodWindow.SetActive(false);
            ...
            if (builder.amountOfBricksCollected >= 1f )
            {
                ...
                if (Physics.Raycast(cameraRay, out cameraRayHit)) // if raycast hits something
                {
                    if (cameraRayHit.transform.CompareTag("Ground")) // if raycast hits the ground
                    {
                        ...
                        if (obj_Array.arrayPos == 5)  // if transpare brick in the rotated position 
                        {
                            transWoodDoor.SetActive(false);
                            transCementBrick.SetActive(false); // set traparent brick to inactive
                            transHalfBrick.SetActive(false);
                            transBrickRotated.SetActive(false); // disable the rotated traparent brick
                            transBrick.SetActive(false); // enable the rotated traparent brick
                            transWoodWindow.SetActive(true);
                            transWoodWindow.transform.position = cameraRayHit.point; // Rotated transparent brick position is where the raycast hits the ground -- this is important as if not set to the ground, the traparent brick can float anywhere between the two raycast position ( raycast location and the hit point)
                ...
                if (obj_Array.arrayPos == 5) // if Brick is in the default postion 
                {
                    // brickForwardDirection = false;
                    //   Debug.Log("This executes");
                    // deleteBrickActive = false;
                    if (Input.GetMouseButtonDown(0) && !UnityEngine.EventSystems.EventSystem.current.IsPointerOverGameObject()) // on mouse click
                    {
                        ...
                        if (Physics.Raycast(position, out hit, 100f)) // if the raycast from position hits in distance of 100f
                        {
                            ...
                            if (hit.transform.CompareTag("Brick"))  // if raycast hits bricks
                            {
                                ...
                                Debug.Log(hit.transform.tag); // log the position it hits
                                                              //agent.destination = hit.point; // move agent (player) to the hit position.
                                Instantiate(woodenWindowToBuildLayer2, hit.point, Quaternion.identity); // create brick upper layer at hitpoint(Upper Bricks) in its original rotation;
                                bricks.Add(woodenWindowToBuildLayer2); // add upper layer bricks to list of bricks
                                builder.Remove1ToBrickAmount();
                            }
                            ...
                            if (hit.transform.CompareTag("Cement"))  // if raycast hits bricks
                            {
                                ...
                                Debug.Log(hit.transform.tag); // log the position it hits
                                                              //agent.destination = hit.point; // move agent (player) to the hit position.
                                Instantiate(woodenWindowToBuild, hit.point, Quaternion.identity); // create brick upper layer at hitpoint(Upper Bricks) in its original rotation;
                                bricks.Add(woodenWindowToBuild); // add upper layer bricks to list of bricks
                                builder.Remove1ToBrickAmount();
                            }
                        }
                        Debug.Log(hit.collider); // log the collider it hits
